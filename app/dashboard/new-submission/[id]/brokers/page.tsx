...
export default function Step2PlatformsAndIRS() {
  ...
  const handleFileUpload = useCallback(async (platformId: string, files: File[]) => {
    ...
    try {
      ...
      try {
        ...
        // If refresh fails, at least clear the loading state and show a warning
        console.error('Error refreshing submission data:', refreshError)
        toast.warning("Files uploaded but failed to refresh", "Please refresh the page to see your uploaded files")
  ...
  const handleTemplateDownload = useCallback(async (platformId: string) => {
    if (platformId === 'manual_log') {
      try {
        ...
        try {
          ...
          const response = await withAuth((accessToken) =>
            apiClient.getManualLogTemplate(accessToken)
          );
          
          downloadUrl = response.template_path;
          ...
          console.warn('API template download failed, using fallback:', apiError);
          
          // Fallback: use direct storage URL
          downloadUrl = "https://storage.googleapis.com/taxsnap-public-data/templates/manual_log_template.csv";
        }
